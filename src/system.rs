...
use crate::app::actor::{AppActor, AppMessage};
use crate::app::data::registry::MIGRATIONS;
use crate::app::data::Db;
use crate::sys::actor::{SysActor, SysMessage};
use crate::sys::util::netlink::Netlink;
use crate::sys::util::tracking::Trackers;
use crate::util::actor::{Actor, AsyncActor, Handle};
...
impl System {
    ...
    pub fn new<T: AsRef<Path>>(path: Option<T>) -> Self {
        ...
        let sys = SysActor { netlink, trackers, app: app_handle.clone() };
...
impl Config {
    pub async fn get() -> &'static Config {
        CONFIG_INSTANCE.get_or_init(|| async {
            Config {
                db_url: String::from("sqlite://sqlite.db"),
                datadir: String::from("/data/lab/rust/quickreplace/src/bin"),
                globalsdir: String::from("/data/lab/rust/quickreplace/src/bin")
            }
        }).await
    }
}
